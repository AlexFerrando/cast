...
class LeashLayer(nn.Module):
    ...
    condition_met_at = defaultdict(lambda: None)
    forward_calls = defaultdict(int)
    condition_layers = None
    behavior_layers = None
    condition_similarities = defaultdict(
    ...
    def steer(
        ...
        condition_projector: torch.Tensor,
        threshold: float = 0.0,
        ...
        **kwargs,
        ...
        log(
            f"    Steering set with apply_behavior_on_first_call: {self.apply_behavior_on_first_call}",
            class_name="LeashLayer",
        )

    def multisteer(
        self,
        behavior_vectors: List[torch.Tensor],
        condition_projectors: List[torch.Tensor],
        ...
        **kwargs,
        ...
        self.params = [
            LayerControlParams(**kwargs) for _ in range(len(behavior_vectors))
        ]
        self.use_ooi_preventive_normalization = use_ooi_preventive_normalization
        self.apply_behavior_on_first_call = apply_behavior_on_first_call
        self.condition_comparator_threshold_is = condition_comparator_threshold_is
        ...
        self.rules = rules
        log(
            f"    Multi-steering set for {len(condition_projectors)} conditions and {len(behavior_vectors)} behaviors",
            class_name="LeashLayer",
        )
    ...
    def forward(self, hidden_states, *args, **kwargs):
        ...
        if not self.is_multi_steering:
            ...
            is_condition_layer = any(
                layers[self.layer_id] for layers in LeashLayer.condition_layers
            )
            is_behavior_layer = any(
                layers[self.layer_id] for layers in LeashLayer.behavior_layers
            )
    ...
    #             # Get the hidden state representation according to mode
    #             if self.condition_threshold_comparison_mode == "mean":
    #                 # Compute cumulative sum along l
    #                 x = hidden_states[i]  # shape (l, d)
    #                 cumsum = torch.cumsum(x, dim=0)  # shape (l, d)

    #                 # Create divisors: 1, 2, ..., l
    #                 lengths = torch.arange(1, x.size(0) + 1, device=x.device).unsqueeze(
    #                     1
    #                 )  # shape (l, 1)

    #                 # Compute cumulative mean
    #                 hidden = cumsum / lengths  # shape (l, d)
    #             elif self.condition_threshold_comparison_mode == "last":
    #                 hidden = hidden_states[i, -1, :]

    #             projected = torch.tanh(
    #                 torch.matmul(self.condition_projector, hidden.T)
    #             ).T
    #             import torch.nn.functional as F

    #             # Cosine similarity between each pair of rows in hidden and projected
    #             similarity = F.cosine_similarity(
    #                 hidden, projected, dim=1
    #             )  # shape: [33]

    #             print(similarity.min(), similarity.max())

    #             if self.condition_comparator_threshold_is == "smaller":
    #                 met_mask = similarity > self.threshold
    #             elif self.condition_comparator_threshold_is == "larger":
    #                 met_mask = similarity < self.threshold

    #             # Find the first index where condition is met
    #             met_indices = torch.nonzero(met_mask, as_tuple=False)

    #             if met_indices.numel() > 0:
    #                 first_met_index = met_indices[0].item()
    #                 met = True
    #             else:
    #                 first_met_index = None
    #                 met = False

    #             LeashLayer.condition_met[i] = met
    #             LeashLayer.condition_met_at[i] = first_met_index
    ...
    def _process_multi_conditions(self, hidden_state):
        ...
        for condition_idx, condition_projector in enumerate(self.condition_projectors):
            if (
                condition_projector is not None
                and not LeashLayer.condition_met[condition_idx]
                and LeashLayer.forward_calls[self.layer_id] == 1
                and LeashLayer.condition_layers[condition_idx][self.layer_id]
            ):
                ...
                projected_hidden_state = torch.tanh(
                    torch.matmul(condition_projector, hidden_state_for_condition)
                )
                condition_similarity = self.compute_similarity(
                    hidden_state_for_condition, projected_hidden_state
                ).item()

                if self.condition_comparator_threshold_is[condition_idx] == "smaller":
                    condition_met = (
                        condition_similarity > self.thresholds[condition_idx]
                    )
                ...
                log(
                    f"    Condition {condition_idx} - Similarity: {condition_similarity}",
                    class_name="LeashLayer",
                )
                log(
                    f"    Condition {condition_idx} - Threshold: {self.thresholds[condition_idx]}",
                    class_name="LeashLayer",
                )
                log(
                    f"    Condition {condition_idx} - Condition Met: {condition_met}",
                    class_name="LeashLayer",
                )
    ...
    #         if should_apply:
    #             start_idx = LeashLayer.condition_met_at[i]
    #             if start_idx is None:
    #                 log(
    #                     f"[Sample {i}] Condition met flag is True but no valid index found.",
    #                     class_name="LeashLayer",
    #                 )
    ...
    #             if LeashLayer.forward_calls[self.layer_id] == 1:
    #                 if self.apply_behavior_on_first_call:
    #                     hidden_states[i, start_idx:] = self.params.operator(
    #                         hidden_states[i, start_idx:], control
    #                     )
    #                     log(
    #                         f"[Sample {i}] Applying behavior vector from index {start_idx} on first call",
    #                         class_name="LeashLayer",
    #                     )
    #                 else:
    #                     log(
    ...
    #                 )
    #                 log(
    #                     f"[Sample {i}] Applying behavior vector from index {start_idx}",
    #                     class_name="LeashLayer",
    #                 )
    ...
    def _apply_multi_behaviors(self, hidden_states):
        ...
        for rule in self.rules:
            ...
            if (
                ...
                and LeashLayer.behavior_layers[behavior_index][self.layer_id]
            ):
                # print(behavior_index)
                log(
                    f"    Rule '{rule}' satisfied. Applying behavior {behavior_index}",
                    class_name="LeashLayer",
                )
                control = self.behavior_vectors[behavior_index].to(
                    dtype=hidden_states.dtype
                )
                if LeashLayer.forward_calls[self.layer_id] == 1:
                    if self.apply_behavior_on_first_call:
                        hidden_states[0] = self.params[behavior_index].operator(
    ...
    def _evaluate_rule(self, rule: str) -> bool:
        ...
        if "or" in conditions:
            return any(
                self._check_single_condition(cond)
    ...
    def _apply_ooi_normalization(self, hidden_states, original_norm):
        ...
        if max_ratio > 1 or has_nan_inf:
            log(
                f"    Applying OOI preventive normalization. Max_ratio was {max_ratio}",
                class_name="LeashLayer",
            )
            hidden_states = hidden_states * (original_norm / new_norm)
        else:
            log(
                f"    No OOI preventive normalization. Max_ratio was {max_ratio}",
                class_name="LeashLayer",
            )
    ...
    def reset_instance(self) -> None:
        ...
        """
        log(
            f"    Resetting LeashLayer @ {self.layer_id} Instance Attributes",
            class_name="LeashLayer",
        )
    ...
    def clean_leashes(cls) -> None:
        ...
        cls.condition_met_at = defaultdict(lambda: None)
