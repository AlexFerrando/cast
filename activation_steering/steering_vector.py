...
class SteeringVector:
    ...
    def load(cls, file_path: str) -> "SteeringVector":
        ...
        explained_variances = {
            int(k): v for k, v in data["explained_variances"].items()
        }

        log(
            f"Loaded directions for layers: {list(directions.keys())}",
            class_name="SteeringVector",
        )
        log(
            f"Shape of first direction vector: {next(iter(directions.values())).shape}",
            class_name="SteeringVector",
        )
...
def read_representations(
    ...
    batch_size: int = 32,
    method: typing.Literal["pca_diff", "pca_center"] = "pca_center",
    ...
    """
    log(
        f"Reading representations for {len(inputs)} inputs", class_name="SteeringVector"
    )
    ...
    if accumulate_last_x_tokens == "all":
        ...
        log(
            f"... accumulating last {accumulate_last_x_tokens} hidden states",
            class_name="SteeringVector",
        )
    ...
    for layer in tqdm(hidden_layer_ids, desc="Reading Hidden Representations ..."):
...
def batched_get_hiddens(
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(batched_inputs, desc="Collecting Hidden Representations ..."):
                    ...
                    if accumulate_last_x_tokens == "all":
                        ...
                        if suffixes:
                            ...
                            suffix_tokens = tokenizer.encode(
                                suffixes[0][0], add_special_tokens=False
                            )
                            ...
                            warnings.warn(
                                "'suffix-only' option used but no suffixes provided. Using last token instead."
                            )
                            accumulated_hidden_state = batch_hidden[-1, :]
                    else:
                        accumulated_hidden_state = torch.mean(
                            batch_hidden[-accumulate_last_x_tokens:, :], dim=0
                        )

                    hidden_states[layer_id].append(
                        accumulated_hidden_state.squeeze().cpu().numpy()
                    )
